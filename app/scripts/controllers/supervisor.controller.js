...
angular.module('ambaya')
    .controller('SupervisorController', ['$scope', 'supervisoresService', 'userService', 'consultoresService', '$stateParams', function ($scope, supervisoresService, userService, consultoresService, $stateParams) {
        ...
        var carregaSupervisor = function () {
        ...
        $scope.modalEditar = function () {
            $scope.edicao = {
                porcentagem: $scope.supervisor.porcentagem
            }
            $('#editar').modal('open');
        }

        $scope.salvarEdicao = function () {
            userService.atualiza($scope.supervisor, $scope.edicao).then(
                function (response) {
                    Materialize.toast("Editado com sucesso!", 5000, 'notificacaoBoa');
                    carregaSupervisor();
                    $('#editar').modal('close');
                },
                function (response) {
                    Materialize.toast("Falha ao editar", 5000, 'notificacaoRuim');
                }
            );
        }
    }]);